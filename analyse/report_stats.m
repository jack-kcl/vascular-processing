...
function [stats M2] = report_stats(name,vox)

%name = 'CFLB3.6i_site1_post';
load([name '.mat']);
...
% rad
stats.segRads = calc_segrads(M2);  % mean+-sd
% leng
M2 = M2.calcSegmentLengths;
stats.leng = M2.getFromSegs('leng');
% vol
stats.segVol = calc_segvol(M2);  % needs checking
% curvature (sum+-sd)
...
%% --
function segRads = calc_segrads(m)
